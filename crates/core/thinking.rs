...
use chrono::{DateTime, Duration as Duration2, Utc, FixedOffset, Local, Timelike, Datelike,
...
fn day_earlier(date_time: DateTime<Utc>) -> Option<DateTime<Utc>> {
    date_time.checked_sub_signed(Duration2::days(1))
}
...
fn do_time() {
    ...
    let almost_three_weeks_from_now = now.checked_add_signed(Duration2::weeks(2))
        .and_then(|in_2weeks| in_2weeks.checked_add_signed(Duration2::weeks(1)))
        .and_then(day_earlier);
    match almost_three_weeks_from_now {
        Some(x) => println!("{}", x),
        None => eprintln!("Almost three weeks from now overflows!"),
    }
    match now.checked_add_signed(Duration2::max_value()) {
        Some(x) => println!("{}", x),
